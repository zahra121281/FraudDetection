...
def add_frauds(customer_profiles_table, terminal_profiles_table, transactions_df, scenario, frauds_number=0):
    ...
    # scenario 4
        ...
        for _ in range(frauds_number):
            ...
            num_small_txs = random.randint(5, 10)
            small_tx_amounts = np.random.uniform(900, 1100, num_small_txs)
            small_tx_amounts = small_tx_amounts / small_tx_amounts.sum() * large_tx['TX_AMOUNT']
            for amount in small_tx_amounts:
                new_tx = large_tx.copy()
                new_tx['TX_AMOUNT'] = amount
                new_tx['TX_FRAUD'] = 1
                new_tx['TX_FRAUD_SCENARIO'] = 5
                transactions_df = transactions_df.append(new_tx, ignore_index=True)
        nb_frauds_scenario_5 = transactions_df['TX_FRAUD'].sum()
        print("Number of frauds from scenario 5: " + str(nb_frauds_scenario_5))
...
if __name__ == "__main__":
    DIR_OUTPUT = input("enter relative output path :")
    absolute_path = os.path.abspath(DIR_OUTPUT)
